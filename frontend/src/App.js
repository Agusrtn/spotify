...
function App() {
  ...
  const regenerateWeeklyTop = async () => {
    if (!user?._id || user.role !== 'admin') return;
    try {
      const res = await fetch(`${API_URL}/admin/regenerate-weekly-top`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ requesterId: user._id }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        showToast(data.error || 'Error al regenerar top semanal', 'error');
        return;
      }
      showToast(data.message, 'success');
    } catch (err) {
      console.error('Error regenerating weekly top:', err);
      showToast('Error al regenerar top semanal', 'error');
    }
  };
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-4 md:mb-6 gap-3">
                  ...
                  <button
                    ...
                    disabled={!radioQueueSongs.length}
                ...
                {radioCurrentSong ? (
                  <div className="flex items-center gap-4 bg-black/40 border border-yellow-400/20 rounded-2xl p-4 mb-4">
                    <div className="w-14 h-14 rounded-xl overflow-hidden bg-black/40 flex-shrink-0">
                      {radioCurrentSong.coverUrl ? <img src={radioCurrentSong.coverUrl} alt={radioCurrentSong.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Disc size={22} className="text-yellow-400/40" /></div>}
                    </div>
                ...
                {user?.role === 'admin' && radioNextSongs.length > 0 && (
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-yellow-400/70 mb-2 flex items-center gap-2">
                      <ListMusic size={13} /> Siguientes en la radio ({radioStation?.queue?.length || 0})
                    </p>
          ...
          <section className="bg-white/5 p-4 md:p-8 rounded-[40px] border border-white/10">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button onClick={regenerateWeeklyTop} className="text-yellow-400 text-xs font-bold hover:underline">Regenerar Top Semanal</button>
